...
pub fn trace(cpu: &mut CPU) -> String {
    ...
    let ops = other_map.get(&code).unwrap();
    ...
    let tmp = match ops.bytes {
        ...
        2 => {
            ...
            match ops.addressing_mode {
                ...
                AddressingMode::NoneAddressing => {
                    // assuming local jumps from things like BNE, BVS, etc.
                    let address: usize = 
                        (begin as usize + 2).wrapping_add((address as i8) as usize);
                    format!("${:04x}", address)
                }
                _ => panic!(
                    "unexpected addressing mode {:?} has ops length 2. code {:02x}",
        ...
        3 => {
            ...
            match ops.addressing_mode {
                ...
                _ => panic!(
                    "unexpected addressing mode {:?} has ops length 3. code {:02x}",
                    ops.addressing_mode, ops.opcode_num
                ),
            }
        }
    ...
    let hex_str = hex_dump
        ...
        .collect::<Vec<String>>()
        .join(" ");
    let asm_str = format!("{:04x}  {:8} {: >4} {}", begin, hex_str, ops.instruction_type, tmp)
        .trim()
        .to_string();
